...
### Agregador universal — Terra (Cluster D) — `lib/wearableAggregator*.ts`, `supabase/functions/wearable-aggregator`

**Una sola conexión cloud que cubre CUALQUIER reloj** (Garmin, Polar, Coros, Suunto, Fitbit, Withings, Samsung, Apple, Google Fit, WHOOP, Oura, +500) y **funciona en web/PWA** sin build nativo — cierra el hueco donde la cobertura cross-marca solo existía en builds nativos. Vendor recomendado tras investigación comparativa (Terra vs Vital/Junction vs Rook vs Spike vs Open Wearables): **Terra** gana por widget hosteado que corre en navegador + compliance líder (HIPAA+BAA+SOC2+GDPR) + free tier ~500 usuarios activos. Runner-up estratégico anti-lock-in: **Open Wearables** (MIT, self-host, $0/usuario) cuando shipee webhooks outbound + 1.0.

- **Lógica pura** (`lib/wearableAggregatorLogic.ts`, 20 tests): arquitectura de **2 capas** para resistir cambio de vendor — `terraToDaily(payload)` (el ÚNICO bloque que conoce el formato Terra) → `AggregatorDaily` (modelo intermedio neutro) → `toWearableDailyRow` (contrato `wearable_daily`). `mergeDailies` fusiona payloads separados del mismo día (sueño/daily/actividad llegan aparte; last-non-null por campo). Cambiar de vendor = reescribir 1 función, no el motor.
- **Edge function** (`supabase/functions/wearable-aggregator/index.ts`): modo **WEBHOOK** (Terra empuja → verifica firma HMAC `terra-signature` fail-closed, dedup por hash del body en `wearable_webhook_events`, resuelve nuestro `user_id` vía `aggregator_user_id`, normaliza, upsert `wearable_daily` provider='aggregator', dispara `calculate-intelligence`) + modo **CONNECT** (app autenticada pide widget session; claves Terra server-side). Maneja eventos `auth` (vincula cuenta vía `reference_id`) / `deauth`.
- **Cliente** (`lib/wearableAggregator.ts`): `connectAggregator()` (web redirige; nativo in-app browser). Reusa hooks `useWearableConnections`/`useWearableDaily` sin cambios. UI: tarjeta "Cualquier reloj" **destacada primera** en `app/perfil/wearables.tsx`.
...
- **Handoffs del dueño** (sin esto el agregador no enruta datos): crear cuenta tryterra.co → `TERRA_DEV_ID`/`TERRA_API_KEY`/`TERRA_SIGNING_SECRET` como **secrets** de Supabase (NUNCA `EXPO_PUBLIC_*`); configurar webhook URL → `…/functions/v1/wearable-aggregator`; **firmar el BAA** antes de enrutar PHI; `supabase functions deploy wearable-aggregator`; aplicar la migración en SQL Editor. **Riesgo a cerrar antes de prod:** el mapeo `terraToDaily` está basado en docs — validar contra un payload Terra REAL (los tests usan fixtures doc-based como mitigación).
