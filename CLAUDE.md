...
supabase functions deploy clickup-onboarding --no-verify-jwt # Alta automática de cliente al firmar en ClickUp (deployed)
```
...
### Focus Desk & rango de mentor — `app/admin/index.tsx`, `app/admin/mentor/[id].tsx`, `hooks/use-admin-role.tsx`

Home nuevo del admin (reemplaza Mission Control): asignar mentor por cliente + un espacio simple para que el mentor trabaje sin la complejidad del panel completo.

- **Rango restringido `is_mentor`** (migración `20260822000000_mentor_role.sql`): un usuario puede ser `is_mentor` sin ser `is_admin` — acceso solo a sus propios clientes, impuesto por **RLS real** vía `public.is_mentor_of(target_user)` (chequea `mentor_assignments`), no cosmético en la UI. Extiende 8 policies existentes (`mentor_tasks`, `mentor_client_scores`, `mentorship_sessions`, `admin_briefings`, `memory_summaries`, `user_memory_profile`, etc.) + 2 policies SELECT-only nuevas (`mentor_assignments`, `user_profiles`). `admin_set_user_role` acepta `'mentor'` como 5to rol (junto a superadmin/admin/premium/inicial), gate = `caller_admin`. Selector de rol en `app/admin/usuarios/[id].tsx`; badge en `usuarios/index.tsx`.
- **`hooks/use-admin-role.tsx`** — `AdminRoleProvider`/`useAdminRole()`: una sola query `is_admin, is_mentor` en `app/admin/_layout.tsx`, consumida por las pantallas hijas sin repetirla. Un mentor logueado ve un chrome liviano (`MentorChrome`, sin el sidebar de 16 secciones) y queda confinado a `/admin` + `/admin/mentor/*` — redirect automático si intenta otra ruta.
- **Focus Desk** (`app/admin/index.tsx`): para admin, 3 lentes (Mi cartera / Asignación / Equipo); para mentor, una sola vista "Mis clientes" con sus propios números (`desk.mine`) — sin las queries org-wide (`fetchUsers`/`fetchMentorsList` se saltan para un mentor). Lógica pura en `lib/admin/deskLogic.ts` (`buildDesk`, testeada) — excluye a los mentores dedicados del universo de "clientes" que ve un admin.
- **Espacio del Mentor** (`app/admin/mentor/[id].tsx`) — modo simple: notas de sesión en un textarea grande con autosave debounced (`adminUpsertSessionNote`), el plan de acción, copiloto IA (`streamClientDesk`), y `ProfileSynopsisCard` + `ConversationTimeline` (reusados de `components/memory.tsx`, vía `fetchUserMemory`) para ver el perfil sintetizado y el resumen de sesiones anteriores sin salir de la pantalla. El link "DOSSIER →" (panel completo) solo se muestra a `role==='admin'`.
- **Copiloto + briefing, resilientes**: `lib/adminCopilot.ts` tiene ahora el mismo fallback tipeado que Norman (`streamDevFallback`/`COPILOT_FALLBACK`) en vez de un string de error crudo cuando los 4 proveedores de IA fallan. `generateAdminBriefing` (`lib/memorySummarizer.ts`) ya no retorna `null` en silencio si la salida del modelo no trae los bloques `===LABEL===` esperados — degrada a usar el texto crudo como resumen; el botón "GENERAR" siempre deja rastro (tarjeta con contenido o mensaje de error visible, no un giro infinito).
...
### Agregador universal — Open Wearables (Cluster D) — `lib/wearableAggregator*.ts`, `supabase/functions/wearable-aggregator`

**Una sola conexión cloud que cubre CUALQUIER reloj** (Garmin, Polar, Coros, Suunto, Fitbit, Withings, Samsung, Apple, Google Fit, WHOOP, Oura, +500) y **funciona en web/PWA** sin build nativo — cierra el hueco donde la cobertura cross-marca solo existía en builds nativos. **Vendor: `open_wearables`, default desde 2026-09-01** (decisión explícita del dueño — "nada de Terra"; MIT, self-host, $0/usuario, sin cuenta comercial ni BAA que firmar). `terra` sigue soportado en el código (`AGGREGATOR_VENDOR=terra` explícito lo reactiva) pero ya NO es el default ni la vía recomendada — se mantiene solo como opción de fallback, no se está usando.

- **Lógica pura** (`lib/wearableAggregatorLogic.ts`, 20 tests): arquitectura de **2 capas** para resistir cambio de vendor — `openWearablesToDaily(payload)` (el bloque que conoce el formato del envelope `{type, data}` de OW) / `terraToDaily(payload)` (Terra, inerte por default) → `AggregatorDaily` (modelo intermedio neutro) → `toWearableDailyRow` (contrato `wearable_daily`). `mergeDailies` fusiona payloads separados del mismo día (sueño/daily/actividad llegan aparte; last-non-null por campo). Cambiar de vendor = reescribir 1 función, no el motor.
- **Edge function** (`supabase/functions/wearable-aggregator/index.ts`): modo **WEBHOOK** — Open Wearables empuja vía Svix (firma HMAC-SHA256 + anti-replay 5min, fail-closed), dedup por hash del body en `wearable_webhook_events`, resuelve nuestro `user_id` vía `aggregator_user_id`, normaliza, upsert `wearable_daily` provider='aggregator', dispara `calculate-intelligence` — + modo **CONNECT** (`generateOpenWearablesAuthUrl`: OAuth POR MARCA, `GET /api/v1/oauth/{provider}/authorize`, claves OW server-side). Maneja eventos `connection.created`/`connection.revoked` (Terra: `auth`/`deauth`, código inerte).
- **Cliente** (`lib/wearableAggregator.ts`): `connectAggregator(provider)` (web redirige; nativo in-app browser) + `disconnectAggregator(provider)`. Reusa hooks `useWearableConnections`/`useWearableDaily` sin cambios. UI: selector de marca en `app/perfil/wearables.tsx` (modo self-host = OAuth por marca, no hay widget multi-marca hosteado).
- **Migración** `20260621000000_wearable_aggregator.sql`: provider `'aggregator'` en el CHECK + columnas `source_device` (reloj real, display) + `aggregator_user_id` (correlación) + tabla `wearable_webhook_events` (idempotencia + auditoría, RLS owner-select).
- **Handoffs del dueño** (sin esto el agregador no enruta datos): **desplegar una instancia de Open Wearables** (stack self-host: Python/FastAPI + Postgres + Redis + Celery — no existe todavía, requiere elegir hosting — Railway/Render/Fly.io/VPS — y desplegarlo; fuera del alcance de este repo) → `OPEN_WEARABLES_BASE_URL`/`OPEN_WEARABLES_API_KEY`/`OPEN_WEARABLES_WEBHOOK_SECRET` como **secrets** de Supabase (NUNCA `EXPO_PUBLIC_*`); configurar el webhook de esa instancia → `…/functions/v1/wearable-aggregator`; `supabase functions deploy wearable-aggregator`; aplicar la migración en SQL Editor; prender `EXPO_PUBLIC_AGGREGATOR_ENABLED=true`. **Riesgo a cerrar antes de prod:** el mapeo `openWearablesToDaily` está basado en docs — validar contra un payload real de una instancia OW.

**WHOOP no depende del agregador.** Tiene su propio OAuth directo (`lib/wearables.ts` + `sync-wearables`, ver sección "Wearables" arriba) — ya está codeado y probado. Para activarlo: `WHOOP_CLIENT_ID`/`WHOOP_CLIENT_SECRET` como secrets de Supabase + registrar `https://growthplayers.vercel.app/oauth/whoop/callback` (web) y `polaris://oauth/whoop/callback` (nativo) en la consola de desarrollador de WHOOP. Es el camino más rápido a "WHOOP funcionando" — no requiere el agregador ni Open Wearables desplegado.

El motor downstream (`lib/biometric.ts`, Confrontation OS, dashboards admin) es provider-agnóstico — consume `wearable_daily` con campos nullable. `sleep_score`/`recovery_score` no vienen universales de HealthKit/HC/Terra; `biometricLogic.computeInsight` ya deriva el estado clínico de HRV+RHR vs baseline. `delete-account` purga `wearable_webhook_events` + Storage PHI `medical-exams`.

### Onboarding automático — ClickUp — `supabase/functions/clickup-onboarding`

Cuando un cliente firma el acuerdo de servicio, su perfil nace solo en Polaris — sin alta manual. Disparador: Automatización de ClickUp en la lista "Polaris" (Servicio → CLIENTES POLARIS), campo custom "Status del contrato" → "Firmado" → POST al webhook. Migración `20260822010000_clickup_onboarding.sql`. Ya desplegada y probada en vivo.

- **Doble idempotencia**: `clickup_webhook_events` (UNIQUE en `sha256(body)`) atrapa reentregas idénticas — retry de ClickUp por timeout; `profiles.clickup_task_id` (UNIQUE parcial) atrapa el mismo `task_id` disparando el webhook otra vez días después (p.ej. alguien reabre "Firmado" por error) — chequeado ANTES de crear el usuario. Mismo patrón que `wearable_webhook_events`.
- **Flujo**: verifica `x-clickup-secret` (comparación en tiempo constante) → dedup de reentrega → dedup de negocio → `auth.admin.createUser` (o engancha por email si el cliente ya tenía cuenta) → `profiles`/`user_profiles`. **`upsert`, no `insert`** — `auth.admin.createUser()` dispara un trigger que ya crea la fila en `profiles`; usar `insert` revienta con `profiles_pkey` duplicado (bug real, encontrado y arreglado en testing en vivo contra prod). Activa membresía **premium** directo — decisión explícita del dueño, sin triage de tier ni mapear el campo "Programa" de ClickUp. Marca el evento procesado.
- **Handoff del dueño**: configurar la Automatización en ClickUp (URL `.../functions/v1/clickup-onboarding`, header `x-clickup-secret`) — el secret ya está en Supabase.

## Key Patterns

**Tablero personalizable (Comando):** el usuario elige hasta 4 métricas de un catálogo de 10 (`metricCatalog` en `app/(tabs)/comando.tsx`). Selección + orden persisten local (respuesta instantánea) y en `profiles.dashboard_metrics` (cross-device; migración `20260716000000_dashboard_metrics.sql`, aplicada). Si la columna no existe, degrada a solo-local. Lógica pura `nextSelection` testeada (FIFO al llenar, mínimo 2) en `hooks/use-dashboard-prefs.ts`. Cada métrica hace drill-down a su pantalla (`route` en el catálogo). Para añadir una métrica nueva: una entrada más en `metricCatalog`, cero cambios en el hook.
...
**Admin panel** (`app/admin/`): two tiers — full `is_admin` and restricted `is_mentor` (own clients only, real RLS — see "Focus Desk & rango de mentor" above), gated in `app/admin/_layout.tsx` with a hardcoded `OWNER_IDS` fallback ~line 134 kept as migration safety net — candidate for removal now that the migration is applied in prod. Never expose admin routes to regular users. Server-side enforcement is the BEFORE-UPDATE anti-escalation trigger + RLS (migrations `20260602000000_security_hardening_p0.sql`, `20260822000000_mentor_role.sql`).

**Admin crea/edita perfiles** (`app/admin/usuarios/`): el admin puede **crear** un usuario real con login (botón "Crear perfil" en `usuarios/index.tsx` → `createUserProfile` en `lib/admin/actions.ts` → edge function `create-user`, que verifica `is_admin` del caller y usa `adminSupabase.auth.admin.createUser`; el tier inicial reusa `activateMembership`) y **editar** nombre/etiqueta-rol (`updateUserProfile` → `user_profiles`, en `usuarios/[id].tsx`). El tier de suscripción se cambia en la sección Membresías. Crear el auth user necesita service-role → vive en edge function (deploy = handoff CLI). El alta también puede ser **automática** — ver "Onboarding automático — ClickUp" en Architecture.
...
### Internista educativo — `lib/internist.ts`, `lib/internistLogic.ts`, `data/internistKnowledge.ts`

IA EDUCATIVA en `app/bienestar/internista.tsx` (Cluster B) — NO es Norman, NO diagnostica. Explica marcadores, fisiología y evidencia de lifestyle medicine citando guías reales (USPSTF, NIH/MedlinePlus, Mayo Clinic, ACLM, Cochrane, Endocrine Society, ADA, ATA, AHA/ACC, KDIGO). Ante cualquier red-flag (dolor torácico, salud mental en crisis, valores críticos de lab, embarazo+medicación, TCA), **deja de educar y deriva** — la lógica pura emite la respuesta de derivación antes de llamar al modelo, así la seguridad no depende del LLM.

- **Base de conocimiento** (`data/internistKnowledge.ts`, datos como constantes): `LAB_MARKERS` con 15 marcadores comunes (glucosa, HbA1c, LDL/HDL/TG, TSH, vit D, B12, ferritina, creatinina, ALT, hs-CRP, testosterona, hemoglobina, potasio) — cada uno con rango low/high, umbrales críticos, qué mide, significado educativo low/high y cita de la guía. `LIFESTYLE_FACTS` con 13 entradas evidenciadas (sueño 7–9h, ejercicio 150min/sem, dieta mediterránea PREDIMED, fuerza 2–3×/sem, creatina, vit D solo si déficit, manejo estrés, ayuno intermitente, HRV, RHR, wearables, omega-3, magnesio) con grado `established|probable|uncertain` + cita. `RED_FLAGS` con 12 reglas (urgent/soon/routine).
- **Lógica pura** (`lib/internistLogic.ts`, 31 tests): `classifyLabValue` (banda `critical_low|low|normal|high|critical_high|unknown` + nota educativa citable), `detectRedFlags` (keyword match con exclude para descartar pasado), `assembleEducationalContext` (empaqueta labs+lifestyle+red-flags para inyectar al prompt), `detectForbiddenLanguage` (guardarraíl de salida: "tienes diabetes", "te receto", etc.). **Está cableado sobre el stream** (`streamInternistResponse`): el chunk que completa la frase no se pinta, se aborta sin reintentar con el siguiente proveedor, y el turno se persiste con el texto correctivo — nunca con el del modelo. Falla cerrado a propósito.
- **IA + IO** (`lib/internist.ts`): `buildInternistPrompt` (system prompt completo con identidad de internista educativo, 6 reglas inviolables, flujo esperado, contexto del paciente, bloques de evidencia citable, disclaimer de cierre obligatorio). `streamInternistResponse` reusa la cadena de fallback endurecida (anthropic→nvidia→groq→openai vía ai-proxy) — **si hay red-flag urgent, responde con derivación sin llamar al modelo** (seguridad determinística). `persistInternistTurn` guarda turnos con snapshot de red-flags detectados. `fetchPatientContext` ensambla perfil + labs históricos + último snapshot biométrico.
- **PHI** (`lib/medicalExams.ts` + migración `20260620000000_internist_phi.sql`): bucket Storage privado `medical-exams/<user_id>/<exam_id>.<ext>` (path enforced por RLS de Storage). Tablas `medical_exams` (metadatos), `medical_lab_values` (marcadores parseados), `internist_sessions` (conversación) — todas RLS owner-only. Admin solo ve metadatos Y SOLO SI `profiles.consents.share_exams_with_coach = true` (gate en SQL, no en cliente). `internist_sessions` es admin-blind por diseño — la conversación con el internista NUNCA llega al coach. Subida es web-first (`<input type="file">` HTML5); nativo muestra notice (handoff: `expo-document-picker`). Archivos servidos por signed URL de 5 min — nunca pública.
- **UI** (`app/bienestar/internista.tsx`): consent gate inicial (persistido en SecureStore/localStorage clave `internist:consent:v1`), barra de disclaimer permanente, chat con quick-prompts, link a "Tus exámenes médicos". `app/bienestar/examenes.tsx`: subida + lista + borrado + toggle "compartir con coach" (escribe a `profiles.consents.share_exams_with_coach`).
- **Líneas rojas codificadas**: (1) red-flag urgent → derivación sin LLM; (2) system prompt prohíbe diagnóstico/prescripción y manda citar fuente; (3) RLS impone owner-only + admin solo con consent explícito; (4) Storage RLS enforce path `<user_id>/...`; (5) bucket privado, jamás público; (6) `internist_sessions` admin-blind.
