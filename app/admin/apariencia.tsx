/**
 * Admin — Apariencia: laboratorio de color en la app REAL.
 *
 * Conmuta los dos ejes de tema (fondo × señal) sobre datos reales, no sobre un
 * mockup. Sirve para decidir con evidencia antes de fijar una paleta.
 *
 * Solo-admin por la ruta (app/admin/_layout.tsx ya exige is_admin). No cambia
 * nada para los clientes: la preferencia vive en el localStorage de ESTE
 * navegador, no en el perfil ni en la base.
 */
...
// ─── Catálogo ─────────────────────────────────────────────────────────────────
// El texto de cada opción dice qué DECIDE, no cómo se ve: el "cómo se ve" ya
// está en pantalla al pulsarlo.

const BACKDROPS: { id: BackdropId; name: string; note: string }[] = [
  { id: 'dark',   name: 'Smoky',  note: 'El manual, literal. Es lo que la app usa hoy.' },
  { id: 'carbon', name: 'Carbón', note: 'El neutro se sesga al oro: menos quirófano, más cuero.' },
  { id: 'aura',   name: 'Aura',   note: 'Negro más profundo + resplandor radial. Atmósfera de bienestar.' },
  { id: 'light',  name: 'Luz',    note: 'El día. Rampa clara con texto Smoky Black.' },
];

const SIGNALS: { id: SignalId; name: string; note: string }[] = [
  { id: 'oro',      name: 'Oro',       note: 'Philippine Yellow para todo. Es el punto de partida actual.' },
  { id: 'ambar',    name: 'Ámbar',     note: 'El segundo oro del manual (#EDBA01). Menos neón, más metal.' },
  { id: 'semaforo', name: 'Semáforo',  note: 'Verde/ámbar/rojo con significado: recuperado · medio · riesgo.' },
  { id: 'calma',    name: 'Dos voces', note: 'El oro exige, el azul restaura. Separa empuje de recuperación.' },
];

/** Muestras de color reales del esquema, leídas del registro — no repintadas a mano. */
  ...
  return (
    <ScrollView
      ...
      <PremiumCard style={s.infoCard}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={s.infoTitle}>LABORATORIO DE COLOR</Text>
          <Text style={s.infoSub}>
            Dos ejes que se combinan: la tinta del fondo y qué comunica el color. Los
            cambios se aplican al instante en toda la app, sobre tus datos reales.
          </Text>
        </View>
      </PremiumCard>

      {/* Honestidad sobre el alcance: sin esto, es fácil creer que ya cambió para
          todos, o que se verá igual en el teléfono. */}
      <View style={s.scopeNote}>
        <MaterialIcons name="info-outline" size={15} color={palette.smoke} />
        <Text style={s.scopeText}>
          {canToggle
            ? 'Solo afecta a este navegador y solo a ti — no cambia lo que ven los clientes ni toca la base de datos. La app nativa (iOS/Android) se queda en Smoky + Oro.'
            : 'En la app nativa los ejes no re-tematizan: el color va compilado en hex. Abre esta pantalla en el escritorio o la PWA para verlos.'}
        </Text>
      </View>
      ...
      <View style={s.grid} accessibilityRole="radiogroup">
        {BACKDROPS.map((b) => (
          <OptionCard
            ...
            note={b.note}
            active={mode === b.id}
            onPress={() => setMode(b.id)}>
            <Swatches
              vars={THEME_VARS[b.id]}
              keys={['--c-bg', '--c-surface', '--c-surface-2', '--c-text']}
            />
          </OptionCard>
        ))}
      </View>
      ...
      <View style={s.grid} accessibilityRole="radiogroup">
        {SIGNALS.map((sig) => (
          <OptionCard
            ...
            name={sig.name}
            note={sig.note}
            active={signal === sig.id}
            onPress={() => setSignal(sig.id)}>
            <Swatches
              vars={SIGNAL_VARS[sig.id]}
              keys={['--c-gold', '--c-success', '--c-warning', '--c-danger']}
            />
          </OptionCard>
        ))}
      </View>
      ...
      <PremiumCard style={s.demoCard}>
        ...
        <View style={s.demoRow}>
          ...
          <View style={[s.demoPill, { backgroundColor: palette.successMuted, borderColor: palette.success, borderWidth: 1 }]}>
            <Text style={[s.demoPillText, { color: palette.success }]}>ÓPTIMO</Text>
          </View>
          <View style={[s.demoPill, { backgroundColor: palette.goldLight, borderColor: palette.warning, borderWidth: 1 }]}>
            <Text style={[s.demoPillText, { color: palette.warning }]}>MEDIO</Text>
          </View>
          <View style={[s.demoPill, { backgroundColor: palette.dangerMuted, borderColor: palette.danger, borderWidth: 1 }]}>
        ...
        <View style={[s.demoRecovery, { borderColor: palette.calm }]}>
          ...
          <Text style={[s.demoRecoveryText, { color: palette.calm }]}>
            Recuperación — cambia de voz solo en “Dos voces”
          </Text>
        </View>
        <Text style={s.demoHint}>
          Estas muestras usan los mismos tokens que el resto de la app, así que lo
          que veas aquí es literalmente lo que verás en Comando y en Biometría.
        </Text>
      </PremiumCard>
