/**
 * app/oauth/strava/callback.tsx
 *
 * Strava OAuth 2.0 callback handler.
 * Strava redirects here after authorization:
 *   GET /oauth/strava/callback?code=...&state=...
 */
  ...
  async function handleCallback() {
    ...
    try {
      const { error: fnError } = await supabase.functions.invoke('sync-wearables', {
        body: { action: 'connect', provider: 'strava', code },
      });

      if (fnError) throw new Error(fnError.message);

      setStatus('success');
      setTimeout(() => router.replace('/perfil/wearables?connected=strava' as never), 1200);
    } catch (e: unknown) {
    ...
    <View style={styles.root}>
      <View style={styles.card}>
        <View style={styles.logoWrap}>
          <MaterialIcons name="directions-run" size={40} color={palette.ash} />
          <Text style={styles.brand}>STRAVA</Text>
        </View>
        ...
        {status === 'loading' && (
          <>
            ...
            <Text style={styles.sub}>Intercambiando tokens con Strava</Text>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            <Text style={styles.sub}>Tu Strava está vinculado. Redirigiendo...</Text>
          </>
        )}
