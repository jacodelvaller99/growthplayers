/**
 * app/oauth/polar/callback.tsx
 *
 * Polar AccessLink OAuth 2.0 callback handler.
 * Polar redirects here after authorization:
 *   GET /oauth/polar/callback?code=...&state=...
 */
  ...
  async function handleCallback() {
    ...
    try {
      const { error: fnError } = await supabase.functions.invoke('sync-wearables', {
        body: { action: 'connect', provider: 'polar', code },
      });

      if (fnError) throw new Error(fnError.message);

      setStatus('success');
      setTimeout(() => router.replace('/perfil/wearables?connected=polar' as never), 1200);
    } catch (e: unknown) {
    ...
    <View style={styles.root}>
      <View style={styles.card}>
        <View style={styles.logoWrap}>
          ...
          <Text style={styles.brand}>POLAR</Text>
        </View>
        ...
        {status === 'loading' && (
          <>
            ...
            <Text style={styles.sub}>Intercambiando tokens con Polar</Text>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            <Text style={styles.sub}>Tu Polar está vinculado. Redirigiendo...</Text>
          </>
        )}
