...
# KDTree con los verts del lado izquierdo (limpio)
kd = KDTree(len(bm.verts))
for v in bm.verts:
    kd.insert(v.co, v.index)
kd.balance()

center = Vector((0.155, 0.01, 0.60))
RADIUS = 0.10
...
for v in bm.verts:
    p = v.co
    if not (0.06 < p.x < 0.27 and -0.12 < p.y < 0.14 and 0.42 < p.z < 0.78):
        continue
    d = (Vector((p.x,p.y,p.z)) - center).length
    if d > RADIUS: continue
    w = smooth(1.0 - d/RADIUS)
    # gemelo espejado en el muslo izquierdo
    co, idx, dist = kd.find((-p.x, p.y, p.z))
    if dist < 0.012 and bm.verts[idx].co.x < -0.05:
        twin = bm.verts[idx].co
        v.co.x = p.x + ((-twin.x) - p.x)*w
        v.co.y = p.y + (( twin.y) - p.y)*w*0.5
        mov += 1
bm.to_mesh(me); bm.free(); me.update()
print("MIRRORED", mov)

# asentar con suavizado local suave
for _ in range(4):
    bm = bmesh.new(); bm.from_mesh(me); bm.verts.ensure_lookup_table()
    sel = [v for v in bm.verts if 0.06<v.co.x<0.27 and -0.12<v.co.y<0.14 and 0.42<v.co.z<0.78]
    for v in sel:
        vs=[e.other_vert(v) for e in v.link_edges]
        if vs:
            avg=Vector((0,0,0))
            for o in vs: avg+=o.co
            v.co = v.co.lerp(avg/len(vs), 0.35)
    bm.to_mesh(me); bm.free(); me.update()
