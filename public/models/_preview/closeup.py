...
def add_light(name, loc, energy, size_=3):
    ld = bpy.data.lights.new(name, 'AREA'); ld.energy = energy; ld.size = size_
...
# (nombre, target, dir, dist)
shots = {
    "hand":   ((0.28, 0.0, 0.90), (0.5, -0.75, 0.25), 0.55),
    "head":   ((0.0, 0.02, 1.68), (0.6, -0.8, 0.15), 0.60),
    "elbow":  ((0.27, -0.01, 1.12), (0.4, -0.8, 0.2), 0.50),
    "knee":   ((0.10, 0.0, 0.52), (0.6, -0.8, 0.1), 0.55),
    "feet":   ((0.0, -0.02, 0.08), (0.3, -0.9, 0.35), 0.70),
}
for name, (target, d, dist) in shots.items():
    ...
    scene.render.filepath = os.path.join(out_dir, f"{tag}_detail_{name}.png")
